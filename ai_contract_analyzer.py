...
Пример: купля-продажа, физлица"""
    ...
    # === ЭТАП 2: Анализ 10 пунктов (с проверкой подсудности) ===
    prompt_analysis = f"""Ты — юрист. Проанализируй договор СТРОГО по 10 пунктам.

КРИТИЧЕСКИ ВАЖНО:
- Выведи РОВНО 10 пунктов. НЕ МЕНЬШЕ! НЕ БОЛЬШЕ!
- КАЖДЫЙ пункт — ОТДЕЛЬНАЯ СТРОКА
- ЗАПРЕЩЕНО склеивать несколько пунктов в одну строку
- ПЕРЕД каждым новым пунктом — ОБЯЗАТЕЛЬНО перенос строки

Используй ТОЧНО эти названия (в этом порядке!):
1. Предмет договора
...
СТАТУСЫ:
✅ — пункт ЕСТЬ и корректно сформулирован
⚠️ — пункт ЕСТЬ, но требует уточнения/доработки
❌ — пункт ПОЛНОСТЬЮ ОТСУТСТВУЕТ в договоре

ВАЖНО:
- Комментарий — от 10 до 20 слов с конкретикой из договора
- Для недвижимости указывай: адрес, площадь, кадастровый номер
- Цитируй пункты договора
- НЕ используй markdown
- НЕ пиши номера пунктов договора (типа "1.1.", "2.1.") — пиши сразу текст
- НЕ пиши служебные фразы

ПРИМЕР ПРАВИЛЬНОГО ОТВЕТА (10 ОТДЕЛЬНЫХ строк!):
✅ 1. Предмет договора: Арендуется нежилое помещение площадью 150 кв.м. по адресу Санкт-Петербург, ул. Ленина, д.10
❌ 2. Цена и порядок расчётов: В договоре отсутствует указание размера арендной платы и порядка её внесения
⚠️ 3. Сроки исполнения: Указан срок передачи помещения, но отсутствуют конкретные даты начала и окончания аренды
✅ 4. Ответственность: Предусмотрена неустойка в размере 0.1% от суммы задолженности за каждый день просрочки
⚠️ 5. Форс-мажор: Перечислены обстоятельства непреодолимой силы, но отсутствует порядок уведомления сторон
✅ 6. Подсудность: Споры передаются на рассмотрение в Арбитражный суд города Санкт-Петербурга
✅ 7. Расторжение: Определены основания досрочного расторжения и порядок уведомления сторон
❌ 8. Персональные данные: Отсутствует раздел об обработке и защите персональных данных сторон
⚠️ 9. Существенные условия: Указаны основные условия, но некоторые формулировки требуют детализации
❌ 10. Односторонние изменения: В договоре полностью отсутствуют положения о порядке одностороннего изменения условий

ИТОГО: X из 10 в порядке, Y требуют внимания, Z отсутствуют.

ЗАПРЕЩЕНО:
- Склеивать два пункта в одну строку
- Использовать markdown
- Писать номера пунктов договора ("1.1.", "2.1.")
- Писать служебные фразы
- Пропускать пункты

Договор:
{text}"""

    # === ЗАЩИТНАЯ ЛОГИКА: повторяем запрос до 3 раз, если пунктов < 10 ===
    analysis = ""
    for attempt in range(3):
        try:
            ...
            # Считаем количество пунктов в ответе
            items_count = len(re.findall(r'^[✅⚠️❌]\s*\d+\.', analysis, re.MULTILINE))

            if items_count >= 10:
                break  # Успех — выходим из цикла
            else:
                print(f"   ⚠️ Попытка {attempt + 1}/3: найдено только {items_count} пунктов, повторяю запрос...")
        except Exception as e:
