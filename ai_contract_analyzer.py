...
Пример: купля-продажа, физлица"""
    ...
    # === ЭТАП 2: Анализ 10 пунктов (РАЗДЕЛЁН НА 2 ЗАПРОСА) ===
    
    # Запрос 1: пункты 1-5
    prompt_analysis_1 = f"""Ты — юрист. Проанализируй договор по 5 пунктам.

Используй ТОЧНО эти названия:
1. Предмет договора
...
✅ 1. Предмет договора: [твой анализ 10-20 слов]
✅ 2. Цена и порядок расчётов: [твой анализ 10-20 слов]
✅ 3. Сроки исполнения: [твой анализ 10-20 слов]
✅ 4. Ответственность: [твой анализ 10-20 слов]
✅ 5. Форс-мажор: [твой анализ 10-20 слов]

ЗАПРЕЩЕНО:
- Склеивать пункты в одну строку
- Использовать markdown
- Писать служебные фразы типа "отмечено ❌", "недочет указан"
...
{text}"""
    ...
    try:
        response = llm.chat(prompt_analysis_1)
        analysis_1 = safe_decode(response.choices[0].message.content)
    except Exception as e:
        analysis_1 = f"❌ Ошибка: {str(e)[:100]}"

    # Запрос 2: пункты 6-10
    prompt_analysis_2 = f"""Ты — юрист. Проанализируй договор по 5 пунктам.
...
ФОРМАТ (РОВНО 5 строк):
✅ 6. Подсудность: [твой анализ 10-20 слов]
✅ 7. Расторжение: [твой анализ 10-20 слов]
✅ 8. Персональные данные: [твой анализ 10-20 слов]
✅ 9. Существенные условия: [твой анализ 10-20 слов]
✅ 10. Односторонние изменения: [твой анализ 10-20 слов]
...
{text}"""
    ...
    try:
        response = llm.chat(prompt_analysis_2)
        analysis_2 = safe_decode(response.choices[0].message.content)
    except Exception as e:
        analysis_2 = f"❌ Ошибка: {str(e)[:100]}"

    # Объединяем результаты
    analysis = f"{analysis_1}\n{analysis_2}"
